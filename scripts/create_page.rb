...
class CreatePage
  PAGE_FILENAME_PATTERN = /\A(?<date>\d{4}-\d{2}-\d{2})-(?<slug>.+)\.md\z/
  ...
  def call
    ...
    slug = parsed_filename.fetch(:slug)
    ...
    template_key = frontmatter.fetch("template", "page")
  ...
  def validate_file!
    ...
    raise ArgumentError, "Filename must look like: 2026-05-20-name-of-page.md" unless parsed_filename
  end

  def parsed_filename
    @parsed_filename ||= begin
      match = File.basename(@path).match(PAGE_FILENAME_PATTERN)
...
if __FILE__ == $PROGRAM_NAME
  path = ARGV.fetch(0) do
    abort "Usage: bundle exec ruby scripts/create_page.rb pages/2026-05-20-name-of-page.md"
  end
