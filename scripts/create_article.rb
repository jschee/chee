...
class CreateArticle
  ARTICLE_FILENAME_PATTERN = /\A(?<date>\d{4}-\d{2}-\d{2})-(?<slug>.+)\.md\z/
  ...
  def call
    ...
    template_key = frontmatter.fetch("template", "article")
  ...
  def validate_file!
    ...
    raise ArgumentError, "Filename must look like: 2026-05-20-name-of-article.md" unless parsed_filename
  end

  def parsed_filename
    @parsed_filename ||= begin
      match = File.basename(@path).match(ARTICLE_FILENAME_PATTERN)
...
if __FILE__ == $PROGRAM_NAME
  path = ARGV.fetch(0) do
    abort "Usage: bundle exec ruby scripts/create_article.rb articles/2026-05-20-name-of-article.md"
  end
